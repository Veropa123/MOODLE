    ...
    try:
        ...
        if len(matches) > 1:
            ...
            for i, (pid, p) in enumerate(matches, start=1):
                print(f"{i}. {p['name']} (Brand: {p['brand']}, Price: {p['price']}, Stock: {p['stock']})")
            try:
...
def view_sales():
    ...
    for s in sales_history:
        try:
            ...
            print(f"Customer: {customer} | Type: {cust_type} | Product: {product} | Brand: {brand} | Qty: {qty} | Discount: {discount:.2f} | Total: ${total:.2f}")
...
def save_sales_csv(ruta="sales_history.csv"):
    ...
    try:
        with open(ruta, mode="w", newline="", encoding="utf-8") as file:
            ...
            # Encabezado correcto
            writer.writerow(["customer", "type", "product", "brand", "qty", "discount", "total"])
